...
export function RatingsBanner() {
  ...
  const headline = status.stale
    ? 'Ratings data is stale — run a fresh extraction'
  ...
  return (
    <div className={`ratings-banner ${status.stale ? 'stale' : 'fresh'}`} role="status">
      ...
      <span className="ratings-banner-text">
        ...
        <small>Last extracted {formatWhen(status.extracted_at)}{age} · source: {status.source}{message ? ` · ${message}` : ''}</small>
      </span>
      <div className="ratings-banner-actions">
        <button className="secondary-button" onClick={importLatest} disabled={busy}>
          <RefreshCw size={14} /> {busy ? 'Importing…' : 'Import latest'}
        </button>
        <button className="banner-primary" onClick={runExtraction}>
          <ExternalLink size={14} /> Run extraction
        </button>
      </div>
    </div>
  )
